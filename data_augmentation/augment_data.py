...
def find_cycle_beat_indices(
    ...
    return cycle_indices, tempo[0], beat_frames

def enhance_sample(y, sr):
    cycle_indices, tempo, beat_frames = find_cycle_beat_indices(y, sr)
    ...
    return new_y
k=0
wav_files = sorted(glob.glob(os.path.join("../old_samples", '*.wav')))
for i,wav_path in enumerate(wav_files):
    # audio_path = f"../old_samples/sample{i}.wav"
    y, sr = librosa.load(wav_path, sr=None)
    batta=y
    for j in range(16):
        new_y = enhance_sample(y, sr)
        
        batta=np.concatenate([batta,new_y])
    sf.write(f'./enhanced2/sample_{k}_enhanced.wav', batta, sr)
    k+=1
    print(f"Sample_{i} is done!")
